...
namespace snmalloc
{
  ...
  public:
    ...
    bool valid_head()
    {
      ...
      auto h = address_cast(free_queue.peek());
      size_t slab_end = (h | ~SLAB_MASK) + 1;
      uintptr_t allocation_start = remove_cache_friendly_offset(h, sizeclass);
    ...
    alloc(Metaslab* self, FreeListIter& fast_free_list, size_t rsize)
    {
      ...
      auto slab = get_slab(self->free_queue.peek());
    ...
    void debug_slab_invariant(Slab* slab)
    {
      ...
      free_queue.terminate();
      FreeListIter fl = free_queue;
