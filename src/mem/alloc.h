...
namespace snmalloc
{
  ...
  public:
    ...
    small_dealloc_unchecked(Superslab* super, void* p, sizeclass_t sizeclass)
    {
      uint8_t chunkmap_slab_kind = chunkmap().get(address_cast(p));
      check_client(
        chunkmap_slab_kind != CMSuperslab,
    ...
    medium_dealloc_unchecked(Mediumslab* slab, void* p, sizeclass_t sizeclass)
    {
      uint8_t chunkmap_slab_kind = chunkmap().get(address_cast(p));
      check_client(
        chunkmap_slab_kind == CMMediumslab,
    ...
    {
      uint8_t claimed_chunkmap_slab_kind = static_cast<uint8_t>(bits::next_pow2_bits(size));

      uint8_t chunkmap_slab_kind = chunkmap().get(address_cast(p));
      check_client(
        chunkmap_slab_kind >= CMLargeMin,
        "Claimed large deallocation is not in a large slab");
      check_client(
        chunkmap_slab_kind == claimed_chunkmap_slab_kind,
