...
namespace snmalloc
{
  ...
  template<size_t K>
  using Arena = BackendArena<MockRep, 0, K>;

  using Bins = BackendArenaBins<2>;
  ...
  template<size_t K>
  static void
  add_and_check(Arena<K>& arena, size_t chunk_idx, size_t size_chunks)
  {
  ...
  template<size_t K>
  static size_t drain_arena(Arena<K>& arena)
  {
    size_t total = 0;
    while (true)
    {
  ...
  template<size_t K>
  using BoundaryArena = BackendArena<BoundaryMockRep, 0, K>;
