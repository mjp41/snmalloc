...
{
  /**
   * Friend struct exposing private internals of `BackendArenaBins<B>`
   * (and its nested `Bitmap`) for unit tests. Forward-declared in
   * `backend_arena_bins.h`; defined here so the production header
   * carries no test-only surface.
   */
  template<size_t INTERMEDIATE_BITS>
  struct BackendArenaBinsTestAccess
  {
    using Bins = BackendArenaBins<INTERMEDIATE_BITS>;
...
{
  using B1 = BackendArenaBinsTestAccess<1>;
  using B2 = BackendArenaBinsTestAccess<2>;
  using B3 = BackendArenaBinsTestAccess<3>;
...
namespace
{
  ...
  {
    using Bins = BackendArenaBinsTestAccess<B>;
  ...
  template<size_t B>
  std::vector<typename BackendArenaBinsTestAccess<B>::chunk_sc_t>
  ...
  size_t reference_find(
    size_t n_chunks, const typename BackendArenaBinsTestAccess<B>::Bitmap& bm)
  {
    using Bins = BackendArenaBinsTestAccess<B>;
    using Bitmap = typename Bins::Bitmap;
